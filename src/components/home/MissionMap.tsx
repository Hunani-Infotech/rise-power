...
export function MissionMap() {
  const { comingSoon } = missionDeployments;
      ...
      >
        ...
        <div className="relative min-h-[16rem] w-full overflow-hidden border border-white/10 sm:min-h-[18rem] lg:min-h-[22rem]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
            <span
              className="inline-flex items-center gap-2 rounded-sm border border-white/15 bg-[#0f140f]/80 px-3 py-1.5 text-[11px] font-semibold tracking-[0.2em] uppercase backdrop-blur-sm"
              style={{ color: sage }}
            >
              <Radar className="size-3.5" strokeWidth={1.75} aria-hidden />
              {comingSoon.badge}
            </span>
            <p className="mt-5 font-display text-4xl leading-none font-bold tracking-tight text-[#f3efe4] uppercase sm:text-5xl lg:text-6xl">
              {comingSoon.panelTitle}
            </p>
            <p className="mt-4 max-w-md text-sm leading-relaxed text-[#f3efe4]/65 sm:text-base">
              {comingSoon.panelBody}
            </p>
          </div>
        </div>
